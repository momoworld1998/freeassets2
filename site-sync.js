...
(function () {
  ...
  try {
    ...
    if (savedAssets && window.ASSETS) {
      ...
      console.log('[site-sync] Loaded ' + live.length + ' assets from admin panel');
    }
    if (savedCats && window.CATEGORIES) {
      const parsed = JSON.parse(savedCats);
      window.CATEGORIES.length = 0;
      parsed.forEach(c => window.CATEGORIES.push(c));
      console.log('[site-sync] Loaded ' + parsed.length + ' categories from admin panel');
    }
    if (savedMusic && window.MUSIC_SAMPLES) {
      ...
      console.log('[site-sync] Loaded ' + parsed.length + ' music tracks from admin panel');
    }
